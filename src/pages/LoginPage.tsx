...
export default function LoginPage() {
  ...
  return (
    <div className="relative min-h-screen bg-background flex items-center justify-center overflow-hidden">
      ...
      <AnimatePresence>
        {scanning && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="text-center space-y-2">
                <motion.div className="flex items-center gap-2 text-primary" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.5, repeat: Infinity }}>
      ...
      <motion.div className="relative z-10 w-full max-w-md mx-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        <motion.button onClick={() => (role ? setRole(null) : navigate("/"))} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-8 transition-colors" whileHover={{ x: -4 }}>
          <ArrowLeft className="w-4 h-4" />{role ? "Change role" : "Back to home"}
        </motion.button>
        ...
        <AnimatePresence mode="wait">
          {!role ? (
            <motion.div key="role-select" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} className="space-y-6">
              <div>
            ...
            <motion.div key="login-form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }}>
              ...
              <form onSubmit={handleDoctorLogin} className="space-y-5">
                <motion.div className="relative h-16 rounded-xl border border-border/30 bg-card/20 backdrop-blur-sm overflow-hidden flex items-center justify-center" whileHover={{ borderColor: "hsl(var(--primary) / 0.4)" }}>
                  ...
                  <div className="relative z-10 flex items-center gap-2 text-xs text-muted-foreground">
                    <Fingerprint className="w-4 h-4 text-primary" />Biometric verification ready
                  </div>
                </motion.div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-foreground">Medical License ID</label>
                  <Input placeholder="MCI-XXXX-XXXXX" className="bg-card/40 backdrop-blur-sm border-border/40 focus:border-primary/60" required />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-foreground">Email</label>
                  <Input type="email" placeholder="dr.singh@hospital.in" className="bg-card/40 backdrop-blur-sm border-border/40 focus:border-primary/60" required />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-foreground">Password</label>
                  <div className="relative">
                    <Input type={showPassword ? "text" : "password"} placeholder="••••••••" className="bg-card/40 backdrop-blur-sm border-border/40 focus:border-primary/60 pr-10" required />
                ...
                <Button type="submit" className="w-full h-12 text-base font-medium">
                  <Fingerprint className="w-4 h-4 mr-2" />Authenticate & Enter
                </Button>
                <p className="text-center text-[11px] text-muted-foreground">Protected by AES-256 encryption · Session expires in 8 hours</p>
              </form>
            </motion.div>
          )}
        </AnimatePresence>
