...
import { Shield, Brain, HeartPulse, Activity, TrendingUp, Dna, Sparkles } from "lucide-react";
    ...
    >
      ...
      <div className="flex items-end gap-2">
        <motion.span
          ...
          className={`text-4xl font-bold font-display ${color}`}
        >
          {score}
        </motion.span>
      ...
      <div className="h-2 rounded-full bg-muted overflow-hidden">
        <motion.div
          className="h-full rounded-full bg-gradient-to-r from-sage via-amber to-coral"
          animate={{ width: `${score}%` }}
...
function PredictiveGraph() {
  ...
  return (
    <motion.div
      className="backdrop-blur-xl bg-card/30 border border-border/30 rounded-2xl p-5"
      whileHover={{ scale: 1.03, boxShadow: "0 0 30px hsl(var(--primary) / 0.15)" }}
      transition={{ type: "spring", stiffness: 300 }}
    >
      <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3">
        <TrendingUp className="w-3.5 h-3.5 text-primary" />
        HbA1c Predictive Trend
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          <p className="text-xs font-medium text-foreground">ShastyaAI Companion</p>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Sparkles className="w-3 h-3 inline mr-1 text-teal" />
          {messages[current]}
        </motion.p>
      </AnimatePresence>
    </motion.div>
...
function VitalsMini() {
  ...
  return (
    <motion.div
      className="backdrop-blur-xl bg-card/30 border border-border/30 rounded-2xl p-5"
      whileHover={{ scale: 1.03, boxShadow: "0 0 30px hsl(var(--coral) / 0.1)" }}
      transition={{ type: "spring", stiffness: 300 }}
    >
      <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
        <HeartPulse className="w-3.5 h-3.5 text-coral" />
        Live Vitals Stream
      </div>
      <div className="flex items-center gap-3">
        <motion.span
          ...
          initial={{ scale: 1.2 }}
          animate={{ scale: 1 }}
          className="text-3xl font-bold font-display text-coral"
        >
          {bpm}
        </motion.span>
      ...
      <div className="flex gap-[3px] mt-2 h-6 items-end">
        {Array.from({ length: 20 }).map((_, i) => (
          <motion.div
            key={i}
            className="w-1.5 rounded-full bg-coral/40"
            animate={{ height: [8, 12 + Math.random() * 12, 8] }}
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        initial={{ opacity: 1 }}
        animate={transitioning ? { opacity: 0, scale: 1.05 } : { opacity: 1 }}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-12 min-h-screen flex flex-col">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-display text-xl text-foreground">ShastyaAI</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center w-full">
              ...
              >
                <div className="space-y-4">
                  <motion.div
                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-teal/10 border border-teal/20 text-xs text-teal font-medium"
                    animate={{ boxShadow: ["0 0 0px hsl(var(--teal) / 0)", "0 0 20px hsl(var(--teal) / 0.15)", "0 0 0px hsl(var(--teal) / 0)"] }}
                ...
                <div className="flex items-center gap-4">
                  <motion.button
                    onClick={handleEnter}
                    className="group relative px-8 py-4 rounded-xl bg-primary text-primary-foreground font-medium text-base overflow-hidden"
                  ...
                  >
                    ...
                    <motion.div
                      className="absolute inset-0 bg-gradient-to-r from-primary via-teal to-primary"
                      animate={{ x: ["-100%", "100%"] }}
                      transition={{ duration: 3, repeat: Infinity }}
                      style={{ opacity: 0.3 }}
                    />
          ...
          {/* Footer stats */}
          <motion.footer
            className="flex flex-wrap items-center justify-center gap-8 py-6 border-t border-border/30"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
